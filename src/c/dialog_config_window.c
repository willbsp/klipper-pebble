...
static void prv_window_load(Window *window) {
  ...
  const int icon_v_center = (bounds.size.h - bitmap_bounds.size.h) / 2;
  const int icon_raise = 20;
  const GEdgeInsets icon_insets =
      GEdgeInsets(icon_v_center - icon_raise, (bounds.size.w - bitmap_bounds.size.w) / 2,
                  icon_v_center + icon_raise);
...
void dialog_config_window_show() {
  if (!s_window) {
